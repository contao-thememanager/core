...
- removed deprecated util-functions `button-padding()` and `shorten-decimal-value()` - use `subList()` and `shortPercentage()` instead
...
- Added *form-checkbox-radio-border-colo*r (border-color for checkbox and radio elements) and their hover and focus state
