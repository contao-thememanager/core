...
class IconGenerator
{
    const string FONTFAMILY_ICON = 'ctm-icon';
    ...
    public function generate($configVars, $xml, FileCompiler|null $compiler): void
    {
        ...
        if (\in_array($iconPath = $this->parseIconFiles(), [null, '', '0'], true)) {
            $this->compiler->msg('Could not create icon font: ' . self::FONTFAMILY_ICON, FileCompiler::MSG_NOTE);
        }
        elseif (($arrGlyphs = $this->parseIcons($iconPath)) === null || ($arrGlyphs = $this->parseIcons($iconPath)) === []) {
     ...
     */
    private function parseIconFiles(): string|null
    {
        ...
        if ((!$absFontPath = realpath($relFontPath = $this->getFilePath($fontPath))) || $relFontPath === '') {
        ...
        foreach ($arrFiles as $file) {
            $arrFileInformation[Path::getExtension($file, true)] = pathinfo($file);
        }
        ...
        if (
            !$this->woffTwo && !\array_key_exists('woff', $arrFileInformation)
     ...
     * @throws \Exception
     */
    private function parseIcons(string $iconPath): array|null
    {
        ...
        $svg = new \SimpleXMLElement($iconPath . '.svg', 0, true);
     ...
     * @throws \Exception
     */
    private function generateIconCSS(array|null $glyphs, string|null $fontPath): string|null
    {
        $css = '';
        ...
        if ($fontPath !== null) {
            ...
            // Add font-source
            $css .= vsprintf(
                "@font-face{font-family:%s;src:url('%s') format('woff%s');%s%s%s}",
                [
